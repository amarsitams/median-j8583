package com.rumango.median.iso.util.impl;
...
public class IsoUtilImpl implements IsoUtil {
	...
	public String packMessage(Map<Integer, String> isoMessage, String isoVersion) throws Exception {
		logger.info("inside packMessage");
		Map<Integer, String> localIsoMsg = new LinkedHashMap<>(isoMessage);
		IsoPojo pojo;
		...
		try {
			...
			IsoMessage m = mfact.newMessage(Integer.parseInt(localIsoMsg.get(1), 16)); // (Integer.parseInt("1200",
			localIsoMsg.remove(1);
			m.setBinaryBitmap(true);
			for (Entry<Integer, String> element : localIsoMsg.entrySet()) {
